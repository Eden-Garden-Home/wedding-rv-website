...
- Palette stays warm cream, muted sage, dusty antique rose, and deep green; avoid very dark full-page green.
- The site opens with an interactive dusty-pink envelope: blind-embossed botanical paper, a centered `V&R` wax seal, and a layered physical opening. The top flap and seal must visibly hinge backward in 3D while the invitation rises from inside; do not represent opening as a hard image swap.
- After opening, the page must continue as the same ivory invitation card inside the pink envelope; do not cut to a separate standalone landing composition.
- Set the visible `Valentina & Riccardo` invitation lettering in the supplied Cherolina script font.
- Continue the invitation with a day timeline, tappable Google Maps cards for the church and reception, a wedding-list/IBAN section, and the RSVP as the final page section.
- Carry a restrained woodland motif through the invitation using fine, realistic botanical branch imagery sourced from a reusable online collection at the paper edges. Render the firefly feeling as many softly animated point-glows with no visible star shape; do not use wired string lights and explicitly avoid fantasy, magical, fairy, gothic, or kitsch styling.
- Keep the full invitation editorial and cohesive: comfortable mobile reading sizes, compact but airy vertical rhythm, softly bordered ivory cards, botanical details held at the paper edges, and one strong deep-green RSVP action.
- Keep RSVP inside the final invitation section instead of opening another sheet, so the response feels like the natural conclusion of the same paper invitation.
...
- On the open-envelope hero, use only a thick, translucent downward arrow as the scroll cue—no label, track, or conventional “Continua sull'invito” pill. Keep it tappable to reach the invitation content. Introduce the same animated arrow as soon as the seal-opening reveal finishes, so it continues seamlessly into the hero.
- Keep the envelope intro accessible: the underlying invitation must be inert and hidden from assistive technology until the intro closes, then focus moves to the scroll cue. Show “Tocca il sigillo” only while the envelope is closed; remove it immediately when opening begins so it never repeats over the revealed invitation. Keep the post-reveal pause brief.
